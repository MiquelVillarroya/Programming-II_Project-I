...
AppStatus Player::Initialise()
{
	...
	sprite->SetAnimationDelay((int)PlayerAnim::TELEPORT_RIGHT, ANIM_DELAY);
	for (i = 0; i < 7; ++i)
		sprite->AddKeyFrame((int)PlayerAnim::TELEPORT_RIGHT, { (float)i*n, 7*n, n, n });

	sprite->SetAnimationDelay((int)PlayerAnim::TELEPORT_LEFT, ANIM_DELAY);
	for (i = 0; i < 7; ++i)
		sprite->AddKeyFrame((int)PlayerAnim::TELEPORT_LEFT, { (float)i*n, 7*n, -n, n });
...
void Player::Teleport(const Point& p) {
	if (look == Look::LEFT) {
		SetAnimation((int)PlayerAnim::TELEPORT_LEFT);
	}
	else if (look == Look::RIGHT) {
		SetAnimation((int)PlayerAnim::TELEPORT_RIGHT);
	}
	SetPos(p);
}
void Player::Update()
